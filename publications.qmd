...
# ---- Table DT (sans filtres colonnes ni recherche globale) ----

dt \<- datatable( pubs \|\> select(title, authors, year, venue, type, `Lien vers la publication`, categories, topcat), colnames = c("Titre","Auteurs","Année","Revue","Type","Lien vers la publication","Mots-clés","topcat"), escape = FALSE, filter = "none", \# pas de filtres de colonnes options = list( pageLength = 50, dom = "tip", \# pas de barre de recherche globale order = list(list(2, 'desc'), list(0, 'asc')), \# Année (col 2, 0-based) desc puis Titre asc columnDefs = list(list(visible = FALSE, targets = 7)), \# cache 'topcat' (index 7) language = list(url = "[https://cdn.datatables.net/plug-ins/1.13.6/i18n/fr-FR.json")](https://cdn.datatables.net/plug-ins/1.13.6/i18n/fr-FR.json")) ), rownames = FALSE )

# ---- Barre de boutons (filtre uniquement via topcat) ----

all_buttons \<- c( list(id="btnAll", label="Voir toutes les publications", value=NA), list(id="btnSM", label="Santé mentale", value="Santé mentale"), list(id="btnDS", label="Déterminants de la santé", value="Déterminants de la santé"), list(id="btnPS", label="Politiques de santé", value="Politiques de santé"), list(id="btnEV", label="Évaluation des systèmes et services de santé", value="Évaluation des systèmes et services de santé"), list(id="btnMV", label="Méthodes de valorisation des données de santé", value="Méthodes de valorisation des données de santé") )
...
```         
  ...
  // 2) Filtre custom sur la colonne cachée 'topcat' (index 7)
  $.fn.dataTable.ext.search.push(function(settings, data, dataIndex) {
    if (selectedCat === null) return true;
    var catCell = api.cell(dataIndex, 7).data();
    var txt = $('<div>').html(catCell).text().trim();   // ex: 'Santé mentale, Évaluation ...'
    var arr = txt.length ? txt.split(/,\\s*/) : [];
    return arr.includes(selectedCat);
  });

  // 3) Gestion des clics + style actif
...
", paste0( sapply(all_buttons, function(b) { sprintf('bar.append('\<button id="%s" class="btn btn-sm %s"\>%s</button>');', b$id,
